    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-[70px]">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="font-micro font-bold text-[12.8px] tracking-wide text-purple uppercase hover:text-purple-dark transition-colors"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          <a
            href="tel:6787494808"
            className="bg-purple text-white p-2.5 hover:bg-purple-dark transition-colors"
            aria-label="Call us"
          >
            <svg
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden bg-white border-t border-mid-gray px-4 py-4 flex flex-col gap-4">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
          ...
          <Link
            href="/#estimate"
            onClick={() => setOpen(false)}
            className="font-micro font-bold text-[13px] uppercase bg-gold text-white px-5 py-2.5 text-center hover:bg-gold/90 transition-colors"
          >
            Free Estimate
          </Link>
        </div>
